...
| Task ID | Title | Description | Dependencies | Supervisor | Complexity | Reference |
|---------|-------|-------------|--------------|------------|------------|-----------|
| P0-E4-T1 | Refactor LineElement base class | Refactor the renamed `LineElement` to support three tiers: pre-built Zag.js machine, custom `createMachine()`, and static (no machine). Lifecycle-managed machine connect/disconnect. Zero overhead for static components. | P0-E1-T6 | Luna | L | ARCH 6, ARCH 8 |
| P0-E4-T2 | Implement Inspector mixin | Review and refactor the existing inspector controller into a mixin on `LineElement`. Feature flag via `localStorage`. Expose metadata: version, docs link, scope, QA tags. Expose CSS parts for inspector overlay. | P0-E4-T1 | Luna | M | PRD 7.2 (Inspector task), Appendix B |
| P0-E4-T3 | Implement Metadata mixin | Mixin that provides component version, documentation URL, and description as properties. Consumed by Inspector mixin. | P0-E4-T1 | Luna | S | ARCH 6 |
| P0-E4-T4 | Implement Direction mixin | Mixin for LTR/RTL detection and `dir` attribute management. | P0-E4-T1 | Luna | S | ARCH 6, PRD 1.7 (RTL) |
